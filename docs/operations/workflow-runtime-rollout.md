...
```json
{
  ...
  "explicit_fallback_policy": "none | equivalent-only"
}
```
...
ANANTA_WORKFLOW_AUTH_KEYRING_SECRET_FILE=/absolute/deployment/path/workflow-auth-keyring.json \
...
- use separate shadow run/evidence correlation generated by the Hub and keep
  ...
  bodies;
- compare only common capabilities and deterministic invariants. Textual model
  differences may be reported but cannot waive security gates.
- persist the content-addressed `ananta.workflow_shadow_comparison.v1` result;
  it compares plan, canonical event, artifact-contract and invariant evidence,
  never raw model text, and is never itself production-eligible.
...
expected source revision. It additionally requires a content-addressed
`ananta.workflow_shadow_comparison.v1` record for the same rollout scope,
target runtime, plan hash and source revision. Set the two absolute read-only
paths with `ANANTA_WORKFLOW_RUNTIME_PERFORMANCE_EVIDENCE_FILE` and
