...
4. `route`: pick execution backend (sgpt, aider, opencode, mistral-code, deerflow) from task intent and policy.
5. `execute`: run planned commands, capture output, gate checks, and trace.
6. `parse` or `artifact_finalize`: normalize backend output into command/tool-call or research artifact payloads.
...
- SGPT and task execution endpoints now surface a `pipeline` object with ordered stages and per-stage metadata.
