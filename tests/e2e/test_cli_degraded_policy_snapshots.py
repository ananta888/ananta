...
def test_cli_degraded_and_policy_snapshots_match_baselines(tmp_path: Path) -> None:
    harness = E2EHarness(artifact_root=tmp_path / "artifacts")
    result = harness.run_cli_degraded_policy(run_id="cli-degraded-001")

    assert set(result.snapshot_refs.keys()) == {
        "backend_unreachable",
        "invalid_auth",
        "approval_required",
        "policy_denied",
