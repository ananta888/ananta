...
def test_terminal_parent_goal_guard_returns_skipped(monkeypatch):
    monkeypatch.setattr(
        "agent.services.task_scoped_execution_service.get_repository_registry",
        lambda: SimpleNamespace(goal_repo=SimpleNamespace(get_by_id=lambda _gid: SimpleNamespace(status="cancelled"))),
    )
    monkeypatch.setattr("agent.services.task_scoped_execution_service.update_local_task_status", lambda *a, **k: None)
