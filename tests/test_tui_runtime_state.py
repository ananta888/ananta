...
def test_tui_view_state_sanitizes_stale_selection() -> None:
    state = TuiViewState(current_section="Tasks").with_selection(goal_id="G-1", task_id="T-1", artifact_id="A-1")
    sanitized = state.sanitize_selection(goal_ids={"G-1"}, task_ids={"T-2"}, artifact_ids=set())
