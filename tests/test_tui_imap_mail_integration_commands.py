...
def _insert_messages(tmp_path: Path) -> None:
...
def test_mail_note_link_artifact_grant_revoke_and_context(monkeypatch, tmp_path: Path) -> None:
    ...
    assert "goal-mail-1:mail://imap-a/INBOX/11" in link_payload["linked_goal_refs"]
    ...
    assert artifact_ref.startswith("mail://imap-a/INBOX/11")
    ...
    assert any(str(item).startswith("mail://imap-a/INBOX/11") for item in local_payload["mail_source_refs"])
