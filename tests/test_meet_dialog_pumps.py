...
def test_late_chat_reply_cannot_cross_a_source_pause_revision():
    page = Mock()
    pump = DialogChatPump(page, Mock(), assignment())
    old_scope = {"generation": 1}
    future = Future()
    pump.opened = old_scope
    pump.revision = 1
...
def test_screen_failure_needs_new_source_consent_and_does_not_reopen_on_tick():
    ...
    source = Mock(source_id="screen:session")
    factory = Mock(return_value=source)
    frames = Mock(busy=False)
    frames.begin.side_effect = ValueError("closed")
    pump = DialogScreenPump(page, Mock(), assignment() | {"capabilities": ["screen.publish"]}, factory, frames=frames)

    def evaluate(expression, *args):
        if "status" in expression:
            return bool(pump.lease)
    ...
    pump.update(control)
    source.take.return_value = "jpeg"
    pump.tick()
    assert pump.failed and pump.source is None
    pump.update(control)
    pump.tick()
    factory.assert_called_once()
    pump.update(control | {"revision": 2})
    assert factory.call_count == 2
    pump.update({"enabled": False, "revision": 3})
    assert pump.source is None
...
def test_screen_setup_failure_is_isolated_and_requires_a_new_control_revision(failure):
    ...
    if failure == "factory":
        factory.side_effect = ValueError("source failed")
    ...
    def evaluate(expression, *args):
        ...
        if failure == "close" and "screen.close" in expression:
            raise ValueError("target gone")
...
def test_partial_audio_setup_closes_browser_lease_and_native_resources(failure, monkeypatch):
    ...
    binding = Mock(return_value=Mock())
    monkeypatch.setattr(module, "bind_subscription", binding)
    pipeline = Mock()
    receiver = Mock()
    pool = Mock()
    ...
    {"binding": binding, "pipeline": pipeline, "receiver": receiver, "pool": pool}[failure].side_effect = ValueError(
        "setup failed"
    )
    page = Mock()
    ...
    if failure in {"receiver", "pool"}:
        pipeline.return_value.cancel.assert_called_once()
    if failure == "pool":
