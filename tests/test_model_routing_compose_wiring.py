...
def test_base_compose_passes_release_control_settings_to_hub():
    environment = _load("docker/compose-next/compose.base.yml")["services"][
        "ai-agent-hub-base"
    ]["environment"]
    ...
    assert environment["FEATURE_LEGACY_MODEL_PICKER_DEPRECATION_ENABLED"] == "${FEATURE_LEGACY_MODEL_PICKER_DEPRECATION_ENABLED:-0}"
    assert environment["MODEL_ROUTING_RELEASE_EVIDENCE_PATH"] == (
        "${MODEL_ROUTING_RELEASE_EVIDENCE_PATH:-/app/artifacts/test-gates/"
        "model-routing-release-gate.json}"
    )
