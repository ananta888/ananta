...
def test_agent_status_validation(app, client):
    ...
    with patch("agent.routes.system.read_json") as mock_read, patch("agent.routes.system.write_json") as mock_write:
...
def test_check_all_agents_health(app):
    ...
    with app.app_context():
        ...
        initial_agents = {
            "agent1": {"url": "http://agent1:5000", "status": "online", "last_seen": time.time() - 100},
            "agent2": {"url": "http://agent2:5000", "status": "online", "last_seen": time.time() - 100},
        }
