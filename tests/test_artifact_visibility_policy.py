...
def test_system_managed_artifacts_are_not_generic(
    ...
    artifact = SimpleNamespace(artifact_metadata={"system_artifact_kind": artifact_kind})
...
def test_generic_artifact_visibility_fails_closed_for_malformed_metadata() -> None:
    ...
    assert is_artifact_visible_on_generic_surfaces(SimpleNamespace(artifact_metadata=["not", "a", "mapping"])) is False
    assert is_artifact_visible_on_generic_surfaces(SimpleNamespace(artifact_metadata={"kind": "user_upload"})) is True
