...
def test_list_tasks_filtering(client, app, admin_auth_header):
    ...
    with app.app_context():
        ...
        # 1. Kein Filter
        response = client.get("/tasks", headers=admin_auth_header)
        assert response.status_code == 200
        tasks = response.json["data"]
        assert len(tasks) >= 3

        # 2. Filter nach Status
        response = client.get("/tasks?status=completed", headers=admin_auth_header)
        assert response.status_code == 200
        tasks = response.json["data"]
        ...
        # 3. Filter nach Agent
        response = client.get("/tasks?agent=agent-a", headers=admin_auth_header)
        assert response.status_code == 200
        tasks = response.json["data"]
        ...
        response = client.get("/tasks?status=todo&agent=agent-a", headers=admin_auth_header)
        assert response.status_code == 200
        tasks = response.json["data"]
        assert len(tasks) == 1
...
def test_list_tasks_time_filtering(client, app, admin_auth_header):
    ...
    with app.app_context():
        ...
        since = now - 60
        response = client.get(f"/tasks?since={since}", headers=admin_auth_header)
        assert response.status_code == 200
        tasks = response.json["data"]
        ...
        until = now - 60
        response = client.get(f"/tasks?until={until}", headers=admin_auth_header)
        assert response.status_code == 200
        tasks = response.json["data"]
...
def test_list_tasks_status_alias_filtering(client, app, admin_auth_header):
    with app.app_context():
...
def test_list_tasks_status_filter_uses_db_query_path(client, app, monkeypatch, admin_auth_header):
    ...
    res = client.get("/tasks?status=completed", headers=admin_auth_header)
