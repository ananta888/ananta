...
def _voice_schema(database: Path) -> tuple[set[str | None], set[str], set[str | None]]:
    ...
    feedback_constraints = {
        item.get("name") for item in inspector.get_unique_constraints("voice_feedback")
    }
    idempotency_columns = {
        item["name"] for item in inspector.get_columns("voice_governance_idempotency")
    }
    idempotency_indexes = {
        item.get("name") for item in inspector.get_indexes("voice_governance_idempotency")
    }
