...
def test_idempotency_key_consistency_on_retries(mock_post):
    ...
    mock_response_success.json.return_value = {"choices": [{"message": {"content": "Erfolgreiche Antwort"}}]}
...
def test_idempotency_key_sent_to_provider(mock_post):
    ...
    mock_response.json.return_value = {"choices": [{"message": {"content": "Test Antwort"}}]}
