...
def test_source_is_isolated_and_only_retains_one_acknowledged_latest_frame():
    ...
    s._receive({"data": "first", "sessionId": 1})
    s._receive({"data": "second", "sessionId": 1})
...
def test_unknown_sources_revoke_before_returning_any_buffered_frame(mutation):
    s, page, context = source()
    s._receive({"data": "sensitive-frame", "sessionId": 1})
    if mutation == "navigation":
        page.url = "https://unapproved.example.test"
    if mutation == "popup":
        context.pages.append(Mock())
    if mutation == "content":
        page.evaluate.return_value = True
    if mutation == "network":
        s._deny(Mock())
    if mutation == "oversize":
        s._receive({"data": "x" * 350001, "sessionId": 1})
...
def test_partial_source_setup_always_closes_its_owned_workspace(failure):
    ...
    target = {
        ...
        "set_content": context.new_page.return_value.set_content,
        "new_cdp_session": context.new_cdp_session,
        "start_screencast": context.new_cdp_session.return_value.send,
    }[failure]
...
def test_unchanged_activity_does_not_mutate_dom_or_add_browser_roundtrips():
    s, page, _ = source()
    state = {"chat": True, "reply": False, "audio": "off"}
    page.evaluate.reset_mock()
    try:
        s.render_activity(state)
        s.render_activity(dict(state))
        page.evaluate.assert_called_once()
        # Capture scalar state, never retain the caller's mutable dictionary.
        state["reply"] = True
        s.render_activity(state)
...
def test_identical_activity_still_rejects_revoked_or_closed_source():
    s, page, _ = source()
    state = {"chat": True, "reply": False, "audio": "off"}
    s.render_activity(state)
    s._revoke()
    with pytest.raises(ValueError, match="workspace_revoked"):
        s.render_activity(state)
    s.close()
    with pytest.raises(ValueError, match="workspace_revoked"):
        s.render_activity(state)
...
def test_failed_activity_rpc_is_not_cached_as_successful():
    s, page, _ = source()
    state = {"chat": True, "reply": False, "audio": "off"}
    page.evaluate.reset_mock()
    page.evaluate.side_effect = [ValueError("browser failed"), None]
    try:
