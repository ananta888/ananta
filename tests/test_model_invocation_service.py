...
def test_implicit_routing_context_disallows_cloud(monkeypatch) -> None:
    ...
    assert any(
        item["reason"] == "security_policy:cloud_disabled_by_routing_context"
        for item in blocked
    )
...
def test_profile_request_applies_generation_limits(
    ...
    assert ModelInvocationService._max_output_tokens_for_request(
        profile,
...
def test_invocation_fallback_chain_local_gemma_qwen(monkeypatch) -> None:
    ...
    assert result["metadata"]["resolution_info"]["initial_profile_id"] == (
        "local_lmstudio_phi_json_worker"
    )
    assert result["metadata"]["resolution_info"]["profile_id"] == (
        "openrouter_qwen3_30b_a3b_stronger"
    )
...
def test_hub_signed_context_overflow_defers_directly_to_hub_recovery(
    ...
    assert (
        raised.value.fallback_decisions[-1]["reason"]
        == "hub_signed_context_recovery_required"
    )
    assert (
        sanitize_terminal_model_recovery_signal(
            raised.value.model_recovery_signal
        )
        is not None
    )
...
def test_bound_primary_context_cannot_authorize_unbound_fallback(
    ...
    assert raised.value.terminal_reason == "policy_blocked"
    assert any(
        decision["reason"] == "provider_fallback_binding_required"
        for decision in raised.value.fallback_decisions
    )
...
def test_public_model_override_is_policy_blocked_when_profiles_are_active(
    ...
    monkeypatch.setattr(
        ...
        "_make_single_chat_call",
        classmethod(
            lambda cls, messages, **kwargs: pytest.fail(
                "model override must fail before provider invocation"
            )
        ),
    )
    ...
    assert raised.value.terminal_reason == "policy_blocked"
    assert raised.value.fallback_decisions[0]["reason"] == (
...
def test_group_budget_allows_two_phi_retries_and_one_gemma_retry(monkeypatch) -> None:
    ...
    phi = ModelProfile(
        profile_id="phi", provider_id="ollama", model="ananta-phi4-mini-32k", local=True,
        retry_budget=2, fallback_group="local", fallback_rank=10,
    )
    gemma = ModelProfile(
        profile_id="gemma", provider_id="ollama", model="ananta-gemma4-reasoning-8k", local=True,
        retry_budget=1, fallback_group="local", fallback_rank=20,
    )
    resolver = ModelProfileResolver(
        [phi, gemma],
        routing_rules=RoutingRules.from_dict(
    ...
    assert calls == (
        ["ananta-phi4-mini-32k"] * 3
        + ["ananta-gemma4-reasoning-8k"] * 2
    )
    ...
    assert result["metadata"]["resolution_info"][
        "fallback_group_max_total_retries"
    ] == 3
...
def test_group_retry_budget_caps_retries_across_all_profiles(monkeypatch) -> None:
    ...
    retry_decisions = [
        decision
        for decision in raised.value.fallback_decisions
        if decision["reason"] == "same_profile_retry_allowed"
    ]
