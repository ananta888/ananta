"""Real bounded loopback HTTP; structural clip doubles, no decoder claim."""
...
KEY = b"synthetic-avatar-video-test-key-32"
...
def endpoint(mode="ok"):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            assert self.headers["X-Ananta-Avatar-Video-Signature"] == video_request_signature(KEY, raw)
            ...
            if mode == "nonce":
                value["nonce"] = "d" * 32
            if mode == "video":
                value["video"]["clip"] = None
            body = json.dumps(value).encode()
            if mode == "oversize":
                body = b" " * (MAX_AVATAR_VIDEO_BYTES + 1)
            if mode == "delay":
            ...
            if mode != "oversize":
                self.send_header("Content-Length", str(MAX_AVATAR_VIDEO_BYTES + 1 if mode == "length" else len(body)))
            signer = response_signature if mode == "legacy_signature" else video_response_signature
            signed = signer(KEY, raw + (b" " if mode == "request_binding" else b""), body)
            self.send_header("X-Ananta-Avatar-Video-Signature", "bad" if mode == "signature" else signed)
            self.end_headers()
            try:
    ...
    try:
        ...
        yield (
            HubAvatarVideoClient(url, KEY, binding, time.monotonic() + 5),
            binding,
            response["video"]["reference"],
            seen,
        )
    finally:
        server.shutdown()
...
def test_real_http_video_response_is_request_bound_and_ignores_environment_proxy(monkeypatch):
    monkeypatch.setenv("http_proxy", "http://127.0.0.1:1")
    with endpoint() as (client, binding, reference, seen):
        result = client.fetch(binding, reference, "loop")
        assert result["reference"] == reference and result["clip"]
        assert len(seen) == 1 and seen[0][0] == "/api/meet/v1/internal/dialog/avatar-video"
...
def test_bad_video_authority_signatures_or_transport_fail_without_retry_or_redirect(mode):
    with endpoint(mode) as (client, binding, reference, seen):
        with pytest.raises(ValueError, match="^meet_avatar_video_hub_revoked_or_unavailable$"):
            client.fetch(binding, reference, "loop")
        assert len(seen) == 1


def test_unauthenticated_video_bytes_are_not_parsed(monkeypatch):
    parser = Mock(side_effect=AssertionError("authenticate before parse"))
    monkeypatch.setattr("worker.meet_media.dialog_avatar_video_client.parse_image_message", parser)
...
def test_hydration_only_uses_fixed_operator_hub_endpoint(url):
    with pytest.raises(ValueError, match="hub_endpoint_required"):
        HubAvatarVideoClient(url, KEY, {}, time.monotonic() + 1)
