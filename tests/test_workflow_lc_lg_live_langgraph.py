...
from worker.core.tool_calling_pipeline import ToolCallOutcome
from worker.core.tool_registry import ToolResult


def _development_langgraph_config(**values) -> LangGraphProviderConfig:
    """Explicit compatibility profile for legacy manual-walker unit tests."""

    metadata = {
        "allow_manual_walker_fallback": True,
        "manual_walker_fallback_mode": "development",
        **dict(values.pop("metadata", {}) or {}),
    }
    values.setdefault("checkpoint_policy", "none")
    return LangGraphProviderConfig(metadata=metadata, **values)
...
def test_lg_adapter_live_path_uses_simplex_for_llm_node(monkeypatch):
    ...
    cfg = _development_langgraph_config(enabled=True, mode="local_live", max_iterations=5)
...
def test_lg_adapter_live_path_uses_langchain_runnable_for_llm_node(monkeypatch):
    ...
    """
    pytest.importorskip(
        "langchain_core.runnables", reason="langchain-core not installed (pip install ananta[langchain])"
    )
    ...
    monkeypatch.setattr(
        rc, "RunnableLambda", staticmethod(lambda fn: type("R", (), {"invoke": staticmethod(lambda x: fn(x))})())
    )
...
def test_lg_adapter_llm_node_records_response_in_state(monkeypatch):
    ...
    with patch("agent.llm_integration.generate_text", side_effect=["out-a", "out-b"]):
        r = a.execute(task_id="t1", task_type="agent_workflow", payload=payload)
    assert r.status == "success"
...
def test_lg_adapter_llm_node_budget_failure_returns_failed(monkeypatch):
    ...
    cfg_for_run = _development_langgraph_config(
        enabled=True,
        mode="local_live",
        max_iterations=5,
        max_nodes=5,
...
def test_lg_tool_node_allowed_tool_proceeds(monkeypatch):
    ...
    cfg = _development_langgraph_config(
        enabled=True,
        mode="local_live",
        max_iterations=5,
    )
    ...
    tool_pipeline = Mock()
    tool_pipeline.execute.return_value = ToolCallOutcome(
        status="success",
    ...
    a = LangGraphAdapter(cfg, tool_pipeline=tool_pipeline)
    ...
    request = tool_pipeline.execute.call_args.args[0]
    assert request.run_id == "t1"
    assert request.tool_id == "search_code"


def test_lg_tool_node_without_pipeline_fails_closed(monkeypatch):
    cfg = _development_langgraph_config(enabled=True, mode="local_live", max_iterations=5)
    adapter = LangGraphAdapter(cfg)
    adapter._policy._allowed_tools = {"search_code"}  # noqa: SLF001
    monkeypatch.setattr(
        "worker.adapters.langgraph_adapter.LangGraphAdapter._langgraph_available",
        staticmethod(lambda: False),
    )
    ...
    assert result.reason_code == "tool_execution_not_configured"
    assert not any(event.get("event") == "tool_node_invoked" for event in result.execution_trace)
...
def test_lg_tool_node_hard_deny_exec_shell_blocked(monkeypatch):
    ...
    payload = {
        "graph_descriptor": {
            "nodes": [{"id": "shell", "kind": "tool", "tool_ref": "exec_shell"}, {"id": "end", "kind": "end"}],
...
def test_lg_tool_node_policy_decision_in_result(monkeypatch):
    ...
    payload = {
        "graph_descriptor": {
            "nodes": [{"id": "t", "kind": "tool", "tool_ref": "unknown_tool"}, {"id": "end", "kind": "end"}],
            "edges": [{"from": "t", "to": "end"}],
        }
    }
    with patch("agent.llm_integration.generate_text", return_value="nope"):
...
def test_lg_dry_run_blocked_when_nodes_exceed_max_nodes():
    ...
    cfg = _development_langgraph_config(enabled=True, mode="dry_run", max_nodes=3)
    a = LangGraphAdapter(cfg)
    nodes = [{"id": f"n{i}", "kind": "llm"} for i in range(5)]
    r = a.dry_run(task_id="t1", task_type="agent_workflow", payload={"graph_descriptor": {"nodes": nodes, "edges": []}})
    assert r.blocked
...
def test_lg_dry_run_not_blocked_when_nodes_within_max_nodes():
    ...
    cfg = _development_langgraph_config(enabled=True, mode="dry_run", max_nodes=5)
    a = LangGraphAdapter(cfg)
...
def test_lg_execute_fails_when_nodes_exceed_max_nodes(monkeypatch):
    ...
    cfg = _development_langgraph_config(enabled=True, mode="local_live", max_nodes=3, max_iterations=10)
    a = LangGraphAdapter(cfg)
    monkeypatch.setattr(
        "worker.adapters.langgraph_adapter.LangGraphAdapter._langgraph_available",
        staticmethod(lambda: False),
    ...
    with patch("agent.llm_integration.generate_text", return_value="ok"):
        r = a.execute(
            task_id="t1", task_type="agent_workflow", payload={"graph_descriptor": {"nodes": nodes, "edges": []}}
        )
...
def test_lg_human_gate_produces_resume_token(monkeypatch):
    ...
    assert r.resume_token is not None, f"Expected resume_token after human_gate stop, got: {r!r}"
...
def test_lg_resume_token_no_secret_pattern(monkeypatch):
    ...
    with patch("agent.llm_integration.generate_text", return_value="sk-SHOULDBEREDACTED12345678"):
        r = a.execute(task_id="t1", task_type="agent_workflow", payload=payload)
...
def test_lg_execute_resume_invalid_token_returns_failed(monkeypatch):
    ...
    r = a.execute(task_id="t1", task_type="agent_workflow", payload={}, resume_token="this is not json{{{")
    assert r.status == "failed"
...
def test_lg_router_takes_conditional_edge_when_condition_matches(monkeypatch):
    ...
    cfg = _development_langgraph_config(enabled=True, mode="local_live", max_iterations=10)
    a = LangGraphAdapter(cfg)
    monkeypatch.setattr(
        "worker.adapters.langgraph_adapter.LangGraphAdapter._langgraph_available",
        staticmethod(lambda: False),
...
def test_lg_router_no_matching_route_stops(monkeypatch):
    ...
    if r.artifacts:
        assert r.artifacts[0].get("stop_reason") in ("no_matching_route", "no_outbound_edge")
...
def test_lg_compiled_graph_failure_blocks_implicit_manual_walker(monkeypatch):
    """A compiled-graph failure must not silently change runtime semantics."""
    ...
    walk_nodes = Mock(wraps=a._walk_nodes)
    monkeypatch.setattr(a, "_walk_nodes", walk_nodes)
    r = a.execute(task_id="t1", task_type="agent_workflow", payload=payload)

    decisions = [e for e in r.execution_trace if e.get("event") == "compiled_graph_fallback_evaluated"]
    assert decisions
    assert decisions[0]["allowed"] is False
    assert decisions[0]["reason_code"] == "runtime_fallback_not_explicitly_enabled"
    assert r.status == "failed"
    assert r.reason_code == "compiled_graph_fallback_blocked"
    assert walk_nodes.call_count == 0


def test_lg_missing_dependency_blocks_implicit_manual_walker(monkeypatch):
    cfg = LangGraphProviderConfig(enabled=True, mode="local_live")
    adapter = LangGraphAdapter(cfg)
    monkeypatch.setattr(
        "worker.adapters.langgraph_adapter.LangGraphAdapter._langgraph_available",
    ...
    walk_nodes = Mock(wraps=adapter._walk_nodes)
    monkeypatch.setattr(adapter, "_walk_nodes", walk_nodes)
    ...
    decision = next(
        ...
        if event.get("event") == "langgraph_dependency_fallback_evaluated"
    )
    assert result.status == "failed"
    assert result.reason_code == "langgraph_not_installed"
    assert decision["fallback_reason_code"] == "langgraph_not_installed"
    assert decision["allowed"] is False
    assert walk_nodes.call_count == 0


def test_lg_manual_walker_fallback_requires_equivalent_explicit_policy(monkeypatch):
    def fake_run_compiled(task_id, task_type, payload, budget):
        raise RuntimeError("simulated compile failure")
    ...
    cfg = _development_langgraph_config(
        ...
        metadata={"allow_manual_walker_fallback": True},
    )
    adapter = LangGraphAdapter(cfg)
    monkeypatch.setattr(
        "worker.adapters.langgraph_adapter.LangGraphAdapter._langgraph_available",
    ...
    monkeypatch.setattr(adapter, "_run_compiled_graph", fake_run_compiled)
    monkeypatch.setattr(
        adapter,
        "_invoke_llm_node",
        lambda **_kwargs: None,
    ...
    result = adapter.execute(task_id="t1", task_type="agent_workflow", payload=payload)

    assert result.status == "success"
    decision = next(
        event for event in result.execution_trace if event.get("event") == "compiled_graph_fallback_evaluated"
    )
    assert decision["allowed"] is True
    assert decision["semantic_class"] == "equivalent"
...
def test_lg_walk_nodes_used_when_langgraph_not_available(monkeypatch):
    ...
    cfg = _development_langgraph_config(enabled=True, mode="local_live")
    a = LangGraphAdapter(cfg)
    monkeypatch.setattr(
        "worker.adapters.langgraph_adapter.LangGraphAdapter._langgraph_available",
        staticmethod(lambda: False),
...
def test_lg_get_checkpointer_returns_none_for_policy_none(monkeypatch):
    ...
    cfg = _development_langgraph_config(enabled=True, mode="local_live", checkpoint_policy="none")
    a = LangGraphAdapter(cfg)
    assert a._get_checkpointer() is None  # noqa: SLF001


def test_lg_get_checkpointer_fails_closed_for_unconfigured_hub_owned(monkeypatch):
    """checkpoint_policy='hub_owned' never silently degrades to no checkpointer."""
    cfg = _development_langgraph_config(enabled=True, mode="local_live", checkpoint_policy="hub_owned")
    a = LangGraphAdapter(cfg)
    with pytest.raises(WorkerError, match="Hub-owned checkpoint policy"):
        a._get_checkpointer()  # noqa: SLF001
...
def test_lg_get_checkpointer_local_ephemeral_no_crash_without_langgraph(monkeypatch):
    ...
    try:
        cfg = _development_langgraph_config(enabled=True, mode="local_live", checkpoint_policy="local_ephemeral")
        a = LangGraphAdapter(cfg)
