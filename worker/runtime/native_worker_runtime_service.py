...
class NativeWorkerRuntimeService:
    ...
    def _native_runtime_cfg(agent_cfg: dict[str, Any] | None) -> dict[str, Any]:
        runtime_cfg = (
            (agent_cfg or {}).get("worker_runtime") if isinstance((agent_cfg or {}).get("worker_runtime"), dict) else {}
        )
        native_cfg = (
            runtime_cfg.get("native_worker_runtime")
            if isinstance(runtime_cfg.get("native_worker_runtime"), dict)
            else {}
        )
    ...
    def shell_policy(self, *, agent_cfg: dict[str, Any] | None) -> dict[str, Any]:
        ...
        allowlist = [
            str(item).strip()
            for item in list(
                configured.get("allowlist")
                or ["python", "pytest", "git", "ls", "pwd", "whoami", "id", "cat", "head", "tail", "wc", "grep", "echo"]
            )
            if str(item).strip()
        ]
        approval_required = [
        ...
        if not normalized_command:
            return {
                ...
                "worker_context_updates": {
                    "native_runtime": {"runtime_path": "native_worker_pipeline", "mode": "command_plan_skipped"}
                },
            }
    ...
    def execute_and_verify_command(
        ...
        *,
        ...
        if not context_hash:
            context_hash = _build_context_hash(
                context_bundle_id=str(
                    (
                        (task.get("worker_execution_context") or {}).get("context_bundle_id")
                        or task.get("context_bundle_id")
                        or f"ctx-{tid}"
                    )
                ),
                task_id=tid,
        ...
        verification_artifact = build_verification_artifact(
            task_id=tid, test_results=[test_result], patch_artifact=None
        )
            ...
            "artifacts": [
                {
                    ...
                    "artifact_ref": f"command:{str(command_plan.get('command_hash') or '')[:16]}",
                },
                {
                    "artifact_type": "test_result_artifact",
                    "artifact_ref": f"test:{status}:{int(test_result.get('exit_code') or 0)}",
                },
                {
                    "artifact_type": "verification_artifact",
                    "artifact_ref": f"verify:{str(verification_artifact.get('status') or 'unknown')}",
                },
            ],
        }
    ...
    def _permission_denied_outcome(
        *, tid: str, trace_id: str, error: str, policy_classification_summary: str
    ) -> dict[str, Any]:
        ...
        *,
        ...
        return {
            ...
            "context_envelope_ref": {
                ...
                "retrieval_refs": [
                    {"source_id": "task_context", "path": f"tasks/{tid}", "reason": "task_scoped_execution"}
                ],
