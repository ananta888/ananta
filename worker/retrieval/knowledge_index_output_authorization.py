...
class KnowledgeIndexWorkerOutputCapabilityAuthorizer:
    ...
    def __init__(
        ...
        *,
        ...
        execution_job_parser: Callable[[Mapping[str, Any]], Any] = (
            parse_execution_job
        ),
    ) -> None:
        ...
        if not self._worker_id or not self._worker_url:
            raise KnowledgeIndexWorkerOutputAuthorizationError(
                "knowledge_index_output_worker_identity_invalid"
            )
    ...
    def authorize(
        ...
        *,
        ...
        assigned_url = str(
            self._value(task, "assigned_agent_url") or ""
        ).strip().rstrip("/")
        if not hmac.compare_digest(assigned_url, self._worker_url):
            self._deny(
        ...
        if (
            str(envelope.get("schema") or "") != _BOUND_JOB_SCHEMA
            or not hmac.compare_digest(
                str(envelope.get("job_id") or ""), normalized_job_id
            )
        ):
            self._deny("knowledge_index_output_execution_binding_invalid")
        embedded_manifest = self._mapping(
            envelope.get("source_access_enforcement_manifest")
        )
        if not self._mapping_equal(embedded_manifest, manifest):
            self._deny(
                "knowledge_index_output_manifest_binding_mismatch",
                status_code=403,
            )
        try:
        ...
        if (
            ...
            or not hmac.compare_digest(
                str(assignment.worker_id), self._worker_id
            )
            or not hmac.compare_digest(
    ...
    @staticmethod
    def _mapping_equal(
        expected: Mapping[str, Any], supplied: Mapping[str, Any]
    ) -> bool:
        try:
        ...
        return hmac.compare_digest(
            ...
            hashlib.sha256(supplied_raw).digest(),
        )
