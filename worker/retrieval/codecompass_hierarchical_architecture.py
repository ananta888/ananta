"""Deterministic hierarchy projection over CodeCompass graph records.

The hierarchy is a view: same snapshot in, same nodes/edges out. Unknown
kinds stay `unknown` instead of being guessed into the wrong level.
"""
...
def classify_level(record: Mapping[str, Any]) -> str:
    ...
    if kind.startswith(("python_", "java_", "ts_")) or tokens & {"function", "class", "method", "symbol", "type"}:
        return "symbol"
...
def project_hierarchy(
    *,
    ...
    for record in list(records or []):
        ...
        title = str(record.get("title") or record.get("name") or record.get("path") or node_id).strip()
        ...
        nodes.append(
            {
                ...
                "title": title,
                "short_summary": str(record.get("summary") or record.get("content") or "")[:240],
                "responsibilities": [
                    item
                ...
                "expandable": classify_level(record) not in {"symbol", "unknown"},
                "parent_id": str(record.get("parent_id") or ""),
                "path": path,
            }
        )
    nodes.sort(key=lambda item: (LEVELS.index(item["level"]) if item["level"] in LEVELS else 9, item["id"]))
    ...
    for edge in list(edges or []):
        ...
        relation, origin = map_relation(str(edge.get("type") or edge.get("relation") or ""))
    ...
    projected_edges.sort(key=lambda item: (item["source"], item["target"], item["relation"], item["id"]))
    return {
