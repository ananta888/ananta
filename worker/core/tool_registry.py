...
def build_default_registry() -> WorkerToolRegistry:
    ...
    _DEFAULTS = [
        ...
        WorkerToolEntry(
            id="codecompass.resolve_context",
            kind="context_retrieval",
            capability_classes=("code_read", "context_resolve", "codecompass_read"),
            risk_class="low",
            input_schema={
                "required": ["query"],
                "properties": {
            ...
            description="Request a policy-bounded CodeCompass ContextPackage from the hub.",
            resource_limits=ResourceLimits(timeout_seconds=20, max_output_chars=32_000, max_files_touched=1),
        ),
        WorkerToolEntry(
            id="codecompass.search_symbols",
            kind="context_search",
            ...
            description="Search CodeCompass symbol/file/domain records.",
            resource_limits=ResourceLimits(timeout_seconds=10, max_output_chars=16_000, max_files_touched=1),
        ),
        WorkerToolEntry(
            id="codecompass.expand_graph",
            kind="graph_expansion",
            capability_classes=("code_read", "codecompass_read", "graph_read"),
            risk_class="low",
            input_schema={
                "required": ["seeds"],
                "properties": {
            ...
            description="Expand bounded CodeCompass graph context.",
            resource_limits=ResourceLimits(timeout_seconds=15, max_output_chars=24_000, max_files_touched=1),
        ),
        WorkerToolEntry(
            id="codecompass.get_file_context",
            kind="file_context",
            capability_classes=("code_read", "codecompass_read", "file_context_read"),
            risk_class="low",
            input_schema={
            ...
            description="Request policy-checked original file excerpts from the hub.",
            resource_limits=ResourceLimits(timeout_seconds=15, max_output_chars=24_000, max_files_touched=8),
        ),
        WorkerToolEntry(
            id="codecompass.get_domain_map",
            kind="domain_map",
            capability_classes=("code_read", "codecompass_read", "domain_read"),
            risk_class="low",
            input_schema={
                "properties": {
                    "domain_hint": {"type": "string"},
