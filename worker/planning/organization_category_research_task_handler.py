...
class OrganizationCategoryResearchTaskHandler:
    ...
    def _prepare(self, *, task: Mapping[str, Any], task_id: str) -> _PreparedExecution:
        ...
        prompt_context = self._prompt_context(chunks)
    ...
    @staticmethod
    def _prompt_context(chunks: list[dict[str, Any]]) -> str:
        """Project verified chunks into a bounded, reference-preserving view."""

        by_source_ref: dict[str, tuple[str, str]] = {}
        for chunk in chunks:
            metadata = OrganizationCategoryResearchTaskHandler._mapping(
                chunk.get("metadata")
            )
            source_ref = str(
                chunk.get("source_id") or metadata.get("source_id") or ""
            ).strip()
            ...
            by_source_ref[source_ref] = (source_label, content)

        entries = sorted(by_source_ref.items())
        if not entries:
            return ""
        headers = [
            f"[{source_ref}] {source_label}"
            for source_ref, (source_label, _content) in entries
        ]
        ...
        if fixed_chars > _PROMPT_CONTEXT_MAX_CHARS:
            ...
            fixed_chars = (
                sum(len(header) + 1 for header in headers) + separator_chars
            )
        if fixed_chars > _PROMPT_CONTEXT_MAX_CHARS:
            raise OrganizationCategoryResearchExecutionError(
                "category_research_prompt_projection_too_large"
            )

        remaining = _PROMPT_CONTEXT_MAX_CHARS - fixed_chars
        excerpt_chars, extra_chars = divmod(remaining, len(entries))
        projected: list[str] = []
        for index, (header, (_source_ref, (_label, content))) in enumerate(
            zip(headers, entries, strict=True)
        ):
