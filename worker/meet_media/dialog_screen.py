...
class OwnedDialogScreen:
    def __init__(self, browser, hub_session_id):
        ...
        self.context = browser.new_context(
            permissions=[], accept_downloads=False, service_workers="block", viewport={"width": 640, "height": 360}
        )
        self.pending = None
        self.closed = False
        self.revoked = False
    ...
    def _start(self):
        ...
        self.page.set_content("""<!doctype html><html><head>
          ...
          <body><h1>Ananta · KI-Arbeitsansicht</h1><p id="activity">Warte auf aktuelle Hub-Freigabe</p>
          <div id="pulse"></div><p id="tick">0</p><script>let n=0;setInterval(()=>{n++;
          document.getElementById('tick').textContent='Laufzeit: '+(n/5).toFixed(1)+' s';
    ...
    def render_activity(self, value):
        ...
        if (
            ...
            or set(value) != {"chat", "reply", "audio"}
            or type(value["chat"]) is not bool
            or type(value["reply"]) is not bool
            or not isinstance(value["audio"], str)
            or value["audio"] not in {"off", "receive", "asr", "reply"}
        ):
            self._revoke()
            raise ValueError("meet_screen_activity_invalid")
        if self.closed or self.revoked:
            raise ValueError("meet_screen_workspace_revoked")
        activity = (value["chat"], value["reply"], value["audio"])
        if activity == self._activity:
            return  # No redundant DOM mutation, screencast work or browser RPC.
        self.page.evaluate(
            """state => {
        ...
        self._activity = activity  # Commit only after the actual browser call succeeds.
    ...
    def take(self):
        ...
        if self.page.evaluate(
            "Boolean(document.querySelector('input,textarea,form,iframe,canvas,video,audio,img,object,embed'))"
        ):
            self.close()
            raise ValueError("meet_screen_content_denied")
