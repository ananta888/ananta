    ...
    public void guidedWorkerSetupCompletesInProot() throws InterruptedException {
        ...
        runIfPresent(
            ...
            "window.__anantaGuidedSetup='PENDING';"
                ...
                + "    var stBefore=await p.getGuidedSetupStatus();"
                + "    if(!(stBefore && stBefore.workspaceInstalled)){"
                + "      window.__anantaGuidedSetup='INSTALL_WORKSPACE';"
                + "      await p.installAnantaWorkspace({});"
                + "      stBefore=await p.getGuidedSetupStatus();"
                + "    }"
                + "    if(!(stBefore && stBefore.workerImportReady && stBefore.workerCommandReady && stBefore.pipReady)){"
    ...
    public void bundledPreseedProvidesWorkerReadySetup() throws InterruptedException {
        ...
        runIfPresent(
            ...
            "window.__anantaPreseedSetup='PENDING';"
                ...
                + "    var workspace;"
                + "    if(stBefore && stBefore.workspaceInstalled){"
                + "      workspace={source:'existing-installation'};"
                + "    } else {"
                + "      workspace=await p.installAnantaWorkspace({});"
                + "      stBefore=await p.getGuidedSetupStatus();"
                + "    }"
                + "    if(!(stBefore && stBefore.workerImportReady && stBefore.workerCommandReady && stBefore.pipReady)){"
                + "      await p.installWorkerDependencies({});"
    ...
    public void wikiPresetImportFlowWorks() throws InterruptedException {
        ...
        runIfPresent(
            ...
            "window.__anantaWikiImport='PENDING';"
                ...
                + "    if(!(presetsResp && presetsResp.ok)){presetsResp=await fetch('http://127.0.0.1:5000/knowledge/wiki/presets',{headers:headers});}"
                + "    if(!(presetsResp && presetsResp.ok)){window.__anantaWikiImport='ERR:PRESETS_HTTP_' + String(presetsResp && presetsResp.status || 'NA');return;}"
                + "    var presetsJson=await presetsResp.json();"
                + "    var data=(presetsJson && presetsJson.data) ? presetsJson.data : presetsJson;"
                + "    var items=Array.isArray(data && data.items) ? data.items : [];"
                + "    var de=items.find(function(item){return String(item&&item.id||'')==='wikipedia-de-multistream-latest';});"
                + "    if(!de){window.__anantaWikiImport='ERR:NO_DE_MULTISTREAM_PRESET';return;}"
                + "    if(String(de.corpus_url||'').indexOf('dewiki-latest-pages-articles-multistream.xml.bz2')<0){window.__anantaWikiImport='ERR:BAD_DE_DUMP_URL';return;}"
                + "    if(String(de.index_url||'').indexOf('dewiki-latest-pages-articles-multistream-index.txt.bz2')<0){window.__anantaWikiImport='ERR:BAD_DE_INDEX_URL';return;}"
                + "    var zim=items.find(function(item){return String(item&&item.id||'')==='wikipedia-de-zim-mini-2026-04';});"
                + "    if(!zim || zim.supported!==false){window.__anantaWikiImport='ERR:ZIM_NOT_MARKED_PROTOTYPE';return;}"
                + "    var zimResp=await fetch('http://127.0.0.1:5000/api/knowledge/wiki/import-url',{"
                + "      method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token},"
                + "      body:JSON.stringify({preset_id:'wikipedia-de-zim-mini-2026-04',async:false})"
                + "    });"
                + "    if(zimResp && zimResp.ok){window.__anantaWikiImport='ERR:ZIM_IMPORT_SHOULD_BE_BLOCKED';return;}"
                + "    window.__anantaWikiImport='OK:' + JSON.stringify({presetId:de.id,indexUrl:de.index_url,items:items.length,zimPrototype:true});"
                + "  }catch(e){"
