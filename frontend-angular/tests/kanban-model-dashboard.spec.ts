...
async function authenticatedFixtures(page: Page): Promise<{ moveCalls: () => number }> {
  ...
  await page.route('**/models/catalog/v1/refresh', route => json(route, {
    schema: 'ananta.model-catalog.v1',
    default_selection: null,
    models: [],
    provider_failures: [],
  }));
  await page.route('**/models/default/v1', route => json(route, {
    schema: 'ananta.model-default-selection.v1',
    provider_id: 'local',
    model_id: 'safe-model',
  }));
  return { moveCalls: () => moves };
}

test('Kanban and model settings stay accessible at desktop and mobile viewports', async ({ page }) => {
  const fixture = await authenticatedFixtures(page);
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto('/board');
  await expect(page.getByTestId('kanban-board')).toBeVisible();
  await page.getByRole('button', { name: 'E2E Karte' }).click();
  await expect(page.getByText('Fixture Kommentar')).toBeVisible();
  await page.getByRole('button', { name: 'Kartendetails schließen' }).click();
  await page.getByRole('button', { name: 'Nach rechts' }).click();
  await expect.poll(fixture.moveCalls).toBe(1);
  await expect(page.getByText(/parallel geändert/)).toBeVisible();
  const axe = await new AxeBuilder({ page })
    .include('.kanban-shell')
  ...
  await expect(page.getByRole('button', { name: 'Als Standard wählen' })).toBeVisible();
