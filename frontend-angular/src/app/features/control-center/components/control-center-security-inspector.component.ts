...
@Component({
  ...
  template: `
    <section class="inspector" *ngIf="policy">
      ...
      <h5>Allowed Paths</h5>
      <ul><li *ngFor="let p of policy.allowedPaths">{{ p }}</li></ul>

      <h5>Denied Paths</h5>
      <ul><li *ngFor="let p of policy.deniedPaths">{{ p }}</li></ul>

      <p class="muted" *ngIf="policy.approvalReason">Grund: {{ policy.approvalReason }}</p>
    </section>
  `,
  styles: [`.inspector{border:1px solid #1f2937;border-radius:10px;padding:10px;background:#0b1220}.row{display:flex;justify-content:space-between;align-items:center;margin:6px 0}.muted{color:#94a3b8;font-size:12px}ul{margin:6px 0 10px 16px}`]
})
