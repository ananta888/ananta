...
export class MobileProotService {
  ...
  buildLoginCommand(distro: string): string {
    ...
    return [
      this.rootfsResolverSnippet(),
      this.prootResolverSnippet(),
      'if [ -f "$ANANTA_PROOT_WRAPPER" ] && [ -d "$ANANTA_ROOTFS" ]; then chmod 755 "$ANANTA_PROOT_WRAPPER" 2>/dev/null || true; if [ -n "$ANANTA_PROOT_BIN" ]; then chmod 755 "$ANANTA_PROOT_BIN" 2>/dev/null || true; fi; export HOME=/root TERM=${TERM:-xterm-256color} PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; ANANTA_WORKDIR="/"; if [ -d "$ANANTA_ROOTFS/root" ]; then ANANTA_WORKDIR="/root"; fi; ANANTA_LOGIN_SHELL=""; if [ -f "$ANANTA_ROOTFS/usr/bin/dash" ]; then ANANTA_LOGIN_SHELL="/usr/bin/dash"; elif [ -f "$ANANTA_ROOTFS/usr/bin/bash" ]; then ANANTA_LOGIN_SHELL="/usr/bin/bash"; elif [ -f "$ANANTA_ROOTFS/usr/bin/sh" ]; then ANANTA_LOGIN_SHELL="/usr/bin/sh"; elif [ -f "$ANANTA_ROOTFS/bin/sh" ]; then ANANTA_LOGIN_SHELL="/bin/sh"; elif [ -f "$ANANTA_ROOTFS/bin/bash" ]; then ANANTA_LOGIN_SHELL="/bin/bash"; elif [ -f "$ANANTA_ROOTFS/bin/dash" ]; then ANANTA_LOGIN_SHELL="/bin/dash"; elif [ -f "$ANANTA_ROOTFS/bin/ash" ]; then ANANTA_LOGIN_SHELL="/bin/ash"; fi; if [ -z "$ANANTA_LOGIN_SHELL" ]; then echo "keine Login-Shell-Datei im rootfs gefunden ($ANANTA_ROOTFS)"; exit 1; fi; echo "proot login shell: $ANANTA_LOGIN_SHELL"; /system/bin/sh "$ANANTA_PROOT_WRAPPER" -r "$ANANTA_ROOTFS" -b /dev:/dev -b /proc:/proc -b /sys:/sys -b "$ANANTA_ROOTFS/usr/lib:/lib" -b "$ANANTA_ROOTFS/usr/bin:/bin" -w "$ANANTA_WORKDIR" -- "$ANANTA_LOGIN_SHELL"; else echo "proot runtime oder rootfs fehlt. Bitte Runtime + Distro installieren."; fi',
    ].join(' && ');
  }
