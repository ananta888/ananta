...
export class MobileProotService {
  ...
  buildWorkerStartInDistroCommand(distro: string): string {
    ...
    return [
      ...
      this.prootResolverSnippet(),
      `if [ -f "$ANANTA_PROOT_WRAPPER" ] && [ -d "$ANANTA_ROOTFS" ]; then chmod 755 "$ANANTA_PROOT_WRAPPER" 2>/dev/null || true; if [ -n "$ANANTA_PROOT_BIN" ]; then chmod 755 "$ANANTA_PROOT_BIN" 2>/dev/null || true; fi; ANANTA_ENV_PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/system/bin:/system/xbin"; ANANTA_WORKDIR="/"; if [ -d "$ANANTA_ROOTFS/root" ]; then ANANTA_WORKDIR="/root"; fi; ANANTA_LOGIN_SHELL=""; if [ -f "$ANANTA_ROOTFS/usr/bin/bash" ]; then ANANTA_LOGIN_SHELL="/usr/bin/bash"; elif [ -f "$ANANTA_ROOTFS/usr/bin/dash" ]; then ANANTA_LOGIN_SHELL="/usr/bin/dash"; elif [ -f "$ANANTA_ROOTFS/usr/bin/sh" ]; then ANANTA_LOGIN_SHELL="/usr/bin/sh"; elif [ -f "$ANANTA_ROOTFS/bin/bash" ]; then ANANTA_LOGIN_SHELL="/bin/bash"; elif [ -f "$ANANTA_ROOTFS/bin/sh" ]; then ANANTA_LOGIN_SHELL="/bin/sh"; elif [ -f "$ANANTA_ROOTFS/bin/dash" ]; then ANANTA_LOGIN_SHELL="/bin/dash"; elif [ -f "$ANANTA_ROOTFS/bin/ash" ]; then ANANTA_LOGIN_SHELL="/bin/ash"; fi; if [ -z "$ANANTA_LOGIN_SHELL" ]; then echo "keine Shell-Datei im rootfs gefunden ($ANANTA_ROOTFS)"; exit 1; fi; ANANTA_PROOT_TMP_BASE=""; for t in /data/user/0/com.ananta.mobile/cache "$ANANTA_PROOT_RUNTIME/tmp"; do if [ -d "$t" ] || mkdir -p "$t" 2>/dev/null; then ANANTA_PROOT_TMP_BASE="$t"; break; fi; done; [ -n "$ANANTA_PROOT_TMP_BASE" ] || { echo "kein beschreibbares temp-verzeichnis fuer proot gefunden"; exit 1; }; ANANTA_PROOT_TMP="$ANANTA_PROOT_TMP_BASE/ananta-proot"; mkdir -p "$ANANTA_PROOT_TMP" 2>/dev/null || true; chmod 700 "$ANANTA_PROOT_TMP" 2>/dev/null || true; PROOT_TMP_DIR="$ANANTA_PROOT_TMP" TMPDIR="$ANANTA_PROOT_TMP" HOME=/root TERM=\${TERM:-xterm-256color} PATH="$ANANTA_ENV_PATH" /system/bin/sh "$ANANTA_PROOT_WRAPPER" -r "$ANANTA_ROOTFS" -b /dev:/dev -b /proc:/proc -b /sys:/sys -b /data:/data -b "$ANANTA_PROOT_TMP:/tmp" -w "$ANANTA_WORKDIR" "$ANANTA_LOGIN_SHELL" -c '${workerStart}'; else echo "proot runtime oder rootfs fehlt. Bitte Runtime + Distro installieren."; exit 1; fi`,
    ].join(' && ');
  }
  ...
  private prootResolverSnippet(): string {
    return [
      ...
      'ANANTA_EMBEDDED_CLASSIC=""',
      'for p in /data/app/*/*/lib/*/libprootclassic.so /data/app/*/lib/*/libprootclassic.so; do if [ -f "$p" ]; then ANANTA_EMBEDDED_CLASSIC="$p"; break; fi; done',
      'if [ -n "$ANANTA_EMBEDDED_CLASSIC" ]; then ANANTA_WRAPPER_TARGET=""; if [ -f "$ANANTA_PROOT_WRAPPER" ]; then ANANTA_WRAPPER_TARGET="$(sed -n \'2s/^exec \"\\([^\"]*\\)\".*/\\1/p\' \"$ANANTA_PROOT_WRAPPER\" 2>/dev/null)"; fi; if [ "$ANANTA_WRAPPER_TARGET" != "$ANANTA_EMBEDDED_CLASSIC" ]; then mkdir -p "$(dirname "$ANANTA_PROOT_WRAPPER")"; printf \'#!/system/bin/sh\\nexec "%s" "$@"\\n\' "$ANANTA_EMBEDDED_CLASSIC" > "$ANANTA_PROOT_WRAPPER"; chmod 755 "$ANANTA_PROOT_WRAPPER" 2>/dev/null || true; fi; fi',
    ].join(' && ');
  }

  private runtimeRootResolverSnippet(): string {
    return 'ANANTA_PROOT_RUNTIME=""; if [ -d "proot-runtime" ]; then ANANTA_PROOT_RUNTIME="proot-runtime"; elif [ -d "$PWD/proot-runtime" ]; then ANANTA_PROOT_RUNTIME="$PWD/proot-runtime"; fi; if [ -z "$ANANTA_PROOT_RUNTIME" ]; then for d in /data/user/0/com.ananta.mobile/files/proot-runtime; do if [ -d "$d" ]; then ANANTA_PROOT_RUNTIME="$d"; break; fi; done; fi; if [ -z "$ANANTA_PROOT_RUNTIME" ]; then ANANTA_PROOT_RUNTIME="/data/user/0/com.ananta.mobile/files/proot-runtime"; fi';
  }
  ...
  private rootfsResolverSnippet(selectedDistro?: string): string {
    ...
    return [
      ...
      'if [ -z "$ANANTA_PROOT_RUNTIME" ]; then for d in /data/user/0/com.ananta.mobile/files/proot-runtime; do if [ -f "$d/bin/proot" ] && [ -d "$d/distros/$ANANTA_DISTRO/rootfs" ]; then ANANTA_PROOT_RUNTIME="$d"; break; fi; done; fi',
      'if [ -z "$ANANTA_PROOT_RUNTIME" ]; then for d in /data/user/0/com.ananta.mobile/files/proot-runtime; do if [ -d "$d" ]; then ANANTA_PROOT_RUNTIME="$d"; break; fi; done; fi',
