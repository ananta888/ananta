...
let SettingsComponent = class SettingsComponent {
    constructor() {
        ...
        this.qgEnabled = true;
        this.qgAutopilotEnforce = true;
        this.qgMinOutputChars = 8;
        this.qgCodingKeywordsText = 'code, implement, fix, refactor, bug, test, feature, endpoint';
        this.qgMarkersText = 'test, pytest, passed, success, lint, ok';
        this.selectedSection = 'llm';
        this.providerCatalog = null;
        this.benchmarkConfig = null;
        this.benchmarkRetentionDays = 90;
        this.benchmarkRetentionSamples = 2000;
        this.benchmarkProviderOrderTextValue = '';
        this.benchmarkModelOrderTextValue = '';
        this.benchmarkValidationError = '';
        this.configRawError = '';
        this.agentLlmDrafts = {};
        this.llmApiKeyProfilesRaw = '{}';
    ...
    getProviderEndpointSummary(provider) {
    ...
    getProviderRuntimeKind(provider) {
        const p = String(provider || '').trim().toLowerCase();
        const baseUrl = this.getBaseUrlForProvider(p);
        if (p === 'lmstudio' || p === 'ollama' || this.getConfiguredLocalBackends().some((entry) => entry.provider === p))
            return 'local runtime';
        if (p === 'codex')
            return this.isProbablyLocalUrl(this.getCodexCliEffectiveBaseUrl()) ? 'local openai-compatible' : 'cloud/openai-compatible';
        if (this.isProbablyLocalUrl(baseUrl))
            return 'local openai-compatible';
        return 'cloud provider';
    }
    getCodexCliEffectiveBaseUrl() {
        ...
        if (codexCfg?.base_url)
            return this.normalizeOpenAICompatibleBaseUrl(codexCfg.base_url);
        if (codexCfg?.prefer_lmstudio !== false)
            return this.normalizeOpenAICompatibleBaseUrl(this.config?.lmstudio_url || 'http://192.168.56.1:1234/v1');
        return this.normalizeOpenAICompatibleBaseUrl(this.config?.openai_url || 'https://api.openai.com/v1/chat/completions');
    }
    getCodexCliTargetSummary() {
    ...
    normalizeOpenAICompatibleBaseUrl(url) {
    ...
    removeLocalOpenAiBackend(index) {
        this.getConfiguredLocalBackends().splice(index, 1);
    }
    normalizeLocalOpenAiBackends(items) {
    ...
    hasApiKey(provider) {
        const llmCfg = this.config?.llm_config || {};
        if (llmCfg?.provider === provider && llmCfg?.api_key)
            return true;
    ...
    getCatalogModels(providerId) {
        const providers = Array.isArray(this.providerCatalog?.providers) ? this.providerCatalog.providers : [];
        const block = providers.find((p) => String(p?.provider || '') === String(providerId || ''));
    ...
    syncQualityGatesFromConfig(cfg) {
