...
let BoardComponent = class BoardComponent {
    constructor() {
        ...
        this.tasks = [];
        this.newTitle = '';
        this.searchText = localStorage.getItem('ananta.board.search') || '';
        this.err = '';
        this.loading = false;
        this.view = localStorage.getItem('ananta.board.view') || 'board';
        this.boardColumns = [
            { id: 'todo', label: 'To-Do' },
        ...
        this.dropListIds = this.boardColumns.map(col => col.id);
        this.normalizeTaskStatus = normalizeTaskStatus;
        this.reload();
    }
    reload() {
        if (!this.hub)
