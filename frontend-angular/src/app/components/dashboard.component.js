...
let DashboardComponent = class DashboardComponent {
    constructor() {
        ...
        this.autopilotGoal = '';
        this.autopilotTeamId = '';
        this.autopilotIntervalSeconds = 20;
        this.autopilotMaxConcurrency = 2;
        this.autopilotBudgetLabel = '';
        this.autopilotSecurityLevel = 'safe';
        this.taskTimeline = [];
        this.benchmarkTaskKind = 'analysis';
        this.benchmarkData = [];
        this.benchmarkUpdatedAt = null;
        this.viewState = { loading: true, error: null, empty: false };
        this.timelineTeamId = '';
        this.timelineAgent = '';
        this.timelineStatus = '';
        this.timelineErrorOnly = false;
        this.quickGoalText = '';
            ...
            next: (rm) => {
                ...
                this.stats = {
                    agents: {
                        ...
                        online: Array.isArray(rm?.agents?.items) ? rm.agents.items.filter((a) => a.status === 'online').length : 0,
                        offline: Array.isArray(rm?.agents?.items) ? rm.agents.items.filter((a) => a.status !== 'online').length : 0,
                    },
                    tasks: {
        ...
        this.hubApi.listAgents(this.hub.url).subscribe({
            next: agents => {
                if (Array.isArray(agents)) {
                    ...
                    this.agentsList = Object.entries(agents).map(([name, info]) => ({
                        name: info.name || name,
                        ...info
                    }));
                }
    ...
    refreshTaskTimeline() {
        if (!this.hub)
            return;
        this.hubApi.getTaskTimeline(this.hub.url, {
    ...
    submitQuickGoal() {
        if (!this.hub || !this.quickGoalText.trim())
            return;
