...
let ArtifactsComponent = class ArtifactsComponent {
    ...
    refresh() {
        ...
        this.hubApi.listArtifacts(this.hub.url).pipe(finalize(() => {
            this.loadingList = false;
        })).subscribe({
            next: (items) => {
    ...
    upload() {
        if (!this.hub || !this.selectedFile)
            return;
        this.uploadBusy = true;
        this.hubApi.uploadArtifact(this.hub.url, this.selectedFile, this.collectionName).pipe(finalize(() => {
            this.uploadBusy = false;
        })).subscribe({
            next: (result) => {
    ...
    selectArtifact(artifactId) {
        if (!this.hub || !artifactId)
            return;
        this.selectedArtifactId = artifactId;
        this.loadingDetail = true;
        this.hubApi.getArtifact(this.hub.url, artifactId).pipe(finalize(() => {
            this.loadingDetail = false;
        })).subscribe({
            next: (payload) => {
    ...
    extractSelected() {
        if (!this.hub || !this.selectedArtifactId)
            return;
        this.extractBusy = true;
        this.hubApi.extractArtifact(this.hub.url, this.selectedArtifactId).pipe(finalize(() => {
            this.extractBusy = false;
        })).subscribe({
            next: () => {
    ...
    knowledgeCollectionNames(payload) {
        const links = Array.isArray(payload?.knowledge_links) ? payload.knowledge_links : [];
        const names = links
            .map((link) => String(link?.link_metadata?.collection_name || link?.collection_id || '').trim())
            .filter((value) => !!value);
        return Array.from(new Set(names));
    }
};
ArtifactsComponent = __decorate([
    Component({
        standalone: true,
...
export { ArtifactsComponent };
//# sourceMappingURL=artifacts.component.js.map