...
let AgentPanelComponent = class AgentPanelComponent {
    constructor() {
    ...
    onPropose() {
        if (!this.agent)
            return;
        this.busy = true;
        this.api.propose(this.agent.url, { prompt: this.prompt }, this.getRequestToken()).pipe(finalize(() => this.busy = false)).subscribe({
            next: (r) => { this.reason = r?.reason || ''; this.command = r?.command || ''; },
            error: () => {
    ...
    onExecute() {
        if (!this.agent || !this.command)
            return;
        this.busy = true;
        this.api.execute(this.agent.url, { command: this.command }, this.getRequestToken()).pipe(finalize(() => this.busy = false)).subscribe({
            next: (r) => {
                this.execOut = r?.output ?? r?.stdout ?? '';
                this.execExit = r?.exit_code ?? r?.exitCode ?? r?.returncode;
                if (this.execExit === 0)
                    this.ns.success('Befehl erfolgreich ausgeführt');
                else
                    this.ns.error(`Befehl fehlgeschlagen (Exit: ${this.execExit})`);
                this.loadLogs();
            },
            error: () => {
    ...
    ensureConfigLoaded() {
        ...
        this.userAuth.token$.pipe(filter((token) => !!token), take(1)).subscribe(() => this.loadConfig());
    }
    loadConfig() {
        if (!this.agent)
            return;
    ...
    saveConfig() {
        ...
        try {
            ...
            this.busy = true;
            this.api.setConfig(this.agent.url, cfg, this.getRequestToken()).pipe(finalize(() => this.busy = false)).subscribe({
                next: () => {
    ...
    saveLLMConfig() {
        ...
        try {
            ...
            this.llmSaving = true;
            this.api.setConfig(this.agent.url, currentCfg, this.getRequestToken()).pipe(finalize(() => this.llmSaving = false)).subscribe({
                next: () => {
            ...
            this.ns.error('Fehler beim Aktualisieren der Konfiguration');
        }
    }
    testLLM() {
        if (!this.agent)
        ...
        this.testResult = '';
        this.api.llmGenerate(this.agent.url, this.testPrompt, this.llmConfig, this.getRequestToken()).pipe(finalize(() => this.llmTesting = false)).subscribe({
    ...
    onRotateToken() {
        if (!this.agent || !this.agent.token)
            return;
        if (!confirm("Token wirklich rotieren? Der aktuelle Token wird sofort ungültig."))
            return;
        this.busy = true;
        this.api.rotateToken(this.agent.url, this.getRequestToken()).pipe(finalize(() => this.busy = false)).subscribe({
            next: (r) => {
                if (this.agent) {
    ...
    loadMetrics() {
        if (!this.agent)
            return;
        this.busy = true;
        this.api.getMetrics(this.agent.url).pipe(finalize(() => this.busy = false)).subscribe({
            next: (m) => this.metrics = m
        });
    }
};
AgentPanelComponent = __decorate([
    Component({
        standalone: true,
...
export { AgentPanelComponent };
//# sourceMappingURL=agent-panel.component.js.map