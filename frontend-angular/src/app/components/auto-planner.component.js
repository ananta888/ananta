...
let AutoPlannerComponent = class AutoPlannerComponent {
    constructor() {
        ...
        this.editingNodeId = '';
        this.editingNode = { title: '', priority: 'Medium' };
    }
    ngOnInit() {
        this.refresh();
    }
    ...
    planGoal() {
        if (!this.hub || !this.goalForm.goal?.trim())
            return;
        this.planning = true;
        this.planningResult = null;
        const body = {
        ...
        this.hubApi.createGoal(this.hub.url, body).subscribe({
            next: (result) => {
                ...
                if (this.selectedGoalId)
                    this.selectGoal(this.selectedGoalId, true);
                this.cdr.detectChanges();
    ...
    selectGoal(goalId, announce = false) {
        if (!this.hub || !goalId)
            return;
        ...
        this.hubApi.getGoalDetail(this.hub.url, goalId).subscribe({
            next: (detail) => {
                ...
                this.cdr.detectChanges();
                if (announce)
                    this.ns.success('Goal-Detail geladen');
            },
            error: (e) => {
                this.selectedGoalDetail = null;
                this.cdr.detectChanges();
    ...
    saveNodeEdit() {
        if (!this.hub || !this.selectedGoalId || !this.editingNodeId)
            return;
...
AutoPlannerComponent = __decorate([
    Component({
        standalone: true,
...
], AutoPlannerComponent);
export { AutoPlannerComponent };
//# sourceMappingURL=auto-planner.component.js.map