...
let TeamsComponent = class TeamsComponent {
    ...
    selectBlueprint(blueprint) {
        ...
        this.blueprintForm = {
            ...
            artifacts: (blueprint.artifacts || []).map((artifact) => ({
                id: artifact.id,
    ...
    saveBlueprint() {
        ...
        const payload = {
            ...
            roles: this.blueprintForm.roles.map((role) => ({
                name: role.name?.trim(),
            ...
            artifacts: this.blueprintForm.artifacts.map((artifact) => ({
                kind: artifact.kind || 'task',
    ...
    deleteBlueprint(id) {
        ...
        if (!this.hub || !confirm('Blueprint wirklich loeschen?'))
            return;
        this.busy = true;
    ...
    onInstantiateBlueprintChange(blueprintId) {
        ...
        this.selectedBlueprintId = blueprint.id;
        if (!this.teamFromBlueprint.name)
            this.teamFromBlueprint.name = `${blueprint.name} Team`;
        if (!this.teamFromBlueprint.description)
            this.teamFromBlueprint.description = blueprint.description || '';
    }
    addInstantiateMember() {
        this.teamFromBlueprint.members.push({ blueprint_role_id: '', agent_url: '', custom_template_id: '' });
    }
    removeInstantiateMember(index) {
    ...
    instantiateBlueprint() {
        ...
        if (!this.hub || !this.teamFromBlueprint.blueprint_id)
            return;
        const payload = {
            ...
            members: this.teamFromBlueprint.members
                .filter((member) => member.agent_url || member.blueprint_role_id || member.custom_template_id)
                .map((member) => ({
    ...
    deleteTeamType(id) {
        ...
        if (!this.hub || !confirm('Team-Typ wirklich loeschen?'))
            return;
    ...
    deleteRole(id) {
        ...
        if (!this.hub || !confirm('Rolle wirklich loeschen?'))
            return;
    ...
    toggleRoleForType(typeId, roleId, isLinked) {
        if (!this.isAdmin) {
            this.ns.error('Admin-Rechte erforderlich');
            return;
        }
    ...
    createTeam() {
        ...
        const payload = {
            ...
            members: (this.newTeam.members || []).map((member) => ({
                agent_url: member.agent_url,
    ...
    deleteTeam(id) {
        ...
        if (!this.hub || !confirm('Team wirklich loeschen?'))
            return;
    ...
    availableAgents(team) {
        const memberUrls = (team.members || []).map((member) => member.agent_url);
        return this.allAgents.filter(agent => !memberUrls.includes(agent.url) && agent.role !== 'hub');
    }
    availableBlueprintAgents() {
        const selectedUrls = this.teamFromBlueprint.members.map((member) => member.agent_url).filter(Boolean);
    ...
    getBlueprintRoleName(team, blueprintRoleId) {
        const role = team?.blueprint_snapshot?.roles?.find((item) => item.id === blueprintRoleId);
