...
class SnakeTickMixin:
    ...
    # ── OIDC / Share ticking (extracted to _snake_tick_share) ────────────────

    def _tick_oidc_device_flow(self, game: dict, *, now: float) -> None:
        _snake_tick_share.tick_oidc_device_flow(self, game, now=now)

    def _tick_e2e_share_autorun(self, game: dict, *, now: float) -> None:
        _snake_tick_share.tick_e2e_share_autorun(self, game, now=now)

    def _get_share_action_futures(self) -> list[Future]:
        return _snake_tick_share.get_share_action_futures(self)

    def _collect_share_action_results(self, game: dict) -> None:
        _snake_tick_share.collect_share_action_results(self, game)
    ...
    def _run_share_action(
        ...
        *args: Any,
        ...
        return _snake_tick_share.run_share_action(self, action, oidc_token, hub_raw, endpoint, *args)

    def _tick_share_pending_action(self, game: dict, *, now: float) -> None:
        _snake_tick_share.tick_share_pending_action(self, game, now=now)

    @staticmethod
    def _resolve_hub_jwt(hub_raw: str, endpoint: str) -> str:
        return _snake_tick_share.resolve_hub_jwt(hub_raw, endpoint)

    @staticmethod
    def _append_share_audit(game: dict, text: str) -> None:
        _snake_tick_share.append_share_audit(game, text)

    def _share_action_create(self, game: dict, oidc_token: str, hub_raw: str, endpoint: str, title: str) -> None:
        _snake_tick_share.share_action_create(self, game, oidc_token, hub_raw, endpoint, title)

    def _share_action_list(self, game: dict, oidc_token: str, hub_raw: str, endpoint: str) -> None:
        _snake_tick_share.share_action_list(self, game, oidc_token, hub_raw, endpoint)

    def _share_action_join(self, game: dict, oidc_token: str, hub_raw: str, endpoint: str, invite_code: str) -> None:
        _snake_tick_share.share_action_join(self, game, oidc_token, hub_raw, endpoint, invite_code)

    def _share_action_set_view(self, game: dict, oidc_token: str, hub_raw: str, endpoint: str, session_id: str, enabled: bool) -> None:
        _snake_tick_share.share_action_set_view(self, game, oidc_token, hub_raw, endpoint, session_id, enabled)

    def _share_action_stop(self, game: dict, oidc_token: str, hub_raw: str, endpoint: str, session_id: str) -> None:
        _snake_tick_share.share_action_stop(self, game, oidc_token, hub_raw, endpoint, session_id)
