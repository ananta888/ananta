"""Scoped expiring discovery handles with independently injected media tables."""

import hashlib
import re
import secrets
import time

from sqlalchemy import BigInteger, Column, MetaData, String, Table, delete, func, insert, select, update
from sqlalchemy.exc import IntegrityError


def cursor_tables(kind):
    if kind not in {"image", "video", "voice"}:
        raise ValueError("persona_cursor_kind_invalid")
    metadata = MetaData()

    def columns():
        return [Column(name, String(255), primary_key=True) for name in ("tenant_id", "project_id", "subject_id")]

    scopes = Table(
        f"persona_{kind}_cursor_scopes", metadata, *columns(), Column("revision", BigInteger, nullable=False)
    )
    cursors = Table(
        f"persona_{kind}_cursors",
        metadata,
        *columns(),
        Column("digest", String(64), primary_key=True),
        Column("position", String(160), nullable=False),
        Column("expires_at", BigInteger, nullable=False),
    )
    return metadata, scopes, cursors


def _where(table, scope):
    return tuple(table.c[name] == value for name, value in scope.items())


class SqlPersonaAssetCursors:
    def __init__(self, engine, *, tables, kind, clock=time.time):
        if kind not in {"image", "video", "voice"}:
            raise ValueError("persona_cursor_kind_invalid")
        self.metadata, self.scopes, self.cursors = tables
        self.kind = kind
        self.engine, self.clock = engine, clock

    def initialize(self):
        self.metadata.create_all(self.engine)

    def resolve(self, *, tenant_id, project_id, subject_id, token):
        if token is None:
            return ""
        if not isinstance(token, str) or not re.fullmatch(r"[A-Za-z0-9_-]{43}", token):
            raise ValueError(f"persona_{self.kind}_cursor_invalid")
        scope = dict(tenant_id=tenant_id, project_id=project_id, subject_id=subject_id)
        with self.engine.connect() as connection:
            position = connection.execute(
                select(self.cursors.c.position).where(
                    *_where(self.cursors, scope),
                    self.cursors.c.digest == hashlib.sha256(token.encode()).hexdigest(),
                    self.cursors.c.expires_at > int(self.clock() * 1000),
                )
            ).scalar_one_or_none()
        if position is None:
            raise ValueError(f"persona_{self.kind}_cursor_unavailable")
        return position

    def issue(self, *, tenant_id, project_id, subject_id, position):
        scope = dict(tenant_id=tenant_id, project_id=project_id, subject_id=subject_id)
        now = int(self.clock() * 1000)
        for attempt in range(2):
            try:
                with self.engine.begin() as connection:
                    # Serialize only this reader's paging state across Hub
                    # replicas; never lock the whole project or asset catalog.
                    changed = connection.execute(
                        update(self.scopes)
                        .where(*_where(self.scopes, scope))
                        .values(revision=self.scopes.c.revision + 1)
                    )
                    if changed.rowcount == 0:
                        connection.execute(insert(self.scopes).values(**scope, revision=1))
                    connection.execute(
                        delete(self.cursors).where(*_where(self.cursors, scope), self.cursors.c.expires_at <= now)
                    )
                    count = connection.execute(
                        select(func.count()).select_from(self.cursors).where(*_where(self.cursors, scope))
                    ).scalar_one()
                    if count >= 128:
                        raise ValueError(f"persona_{self.kind}_cursor_budget_exceeded")
                    token = secrets.token_urlsafe(32)
                    connection.execute(
                        insert(self.cursors).values(
                            **scope,
                            digest=hashlib.sha256(token.encode()).hexdigest(),
                            position=position,
                            expires_at=now + 300_000,
                        )
                    )
                return token
            except IntegrityError:
                if attempt:
                    raise ValueError(f"persona_{self.kind}_cursor_conflict") from None
        raise ValueError(f"persona_{self.kind}_cursor_unavailable")
