...
class IntegrationRegistryService:
    ...
    def normalize_runtime_endpoint_descriptor(
        ...
        *,
        ...
        return normalize_runtime_endpoint_descriptor(
    ...
    def list_exposure_adapters(self, *, cfg: dict[str, Any] | None) -> list[dict[str, Any]]:
        ...
        policies = exposure_service.normalize_exposure_policy(
            get_platform_governance_service().resolve_exposure_policy(cfg)
        )
        ...
        *,
        ...
        for backend in get_local_openai_backends(
            ...
            remote_hub_allowed = (
                (not is_remote_hub)
                or (
                    bool(remote_hubs_policy.get("enabled"))
                    and bool(routing_fallback_policy.get("allow_remote_hubs", True))
                )
            )
            providers.append(
                {
