...
from agent.services.meet_dialog_controls import initial_controls, change_controls, chat_policy_revision
...
class CurrentDialogChatAuthority:
    ...
    def current(self, session_id):
        ...
        if session_id != scope.session_id or not scope.controls.chat.enabled or not {"chat.read", "chat.send"} <= set(scope.capabilities):
            return None
        ...
        return AuthorizedChatSession(ChatScope(origin=scope.origin, tenant_id=scope.tenant_id, project_id=scope.project_id,
            ...
            generation=lease["generation"], room_id=scope.room_id, membership_epoch=receipt["membershipEpoch"],
            policy_revision=chat_policy_revision(receipt["receiveRevision"], scope.controls.chat.revision), own_peer_id=receipt["peerId"],
...
class MeetDialogService:
    def __init__(self, authority, tasks, meet, issuer, worker, media_worker, reservations, dispatches, clock=time.time, media_tasks=None):
        self.authority, self.tasks, self.meet, self.issuer = authority, tasks, meet, issuer
        self.worker, self.media_worker, self.reservations, self.dispatches, self.clock = worker, media_worker, reservations, dispatches, clock
        ...
        self.audio_coordinator = MeetDialogAudio(authority, tasks, meet, reservations, dispatches,
                                                media_worker, self.media_tasks, clock)
    ...
    def list(self, principal, project, cursor=0):
        ...
        for task in rows:
            ...
            if task.tenant_id == principal.tenant_id and task.project_id == project and context.get("owner_subject") == principal.subject_id:
                items.append(self.inspect(principal, project, task.id))
        return {"schema": "ananta.meet-dialog-list.v1", "items": items,
    ...
    def start(self, principal, project, payload, parent=""):
        ...
        if (not isinstance(payload, dict) or set(payload) - {"audio_mode"} != {"capabilities", "duration_seconds", "chat_mode"}
                or type(payload["duration_seconds"]) is not int or not 30 <= payload["duration_seconds"] <= 7200
                or not isinstance(payload["capabilities"], list) or not payload["capabilities"]
                or any(not isinstance(v, str) for v in payload["capabilities"])
                or len(set(payload["capabilities"])) != len(payload["capabilities"])
                or not set(payload["capabilities"]) <= self.authority.policies.get((principal.tenant_id, project), frozenset())):
            raise MeetError("meet_dialog_start_denied", 403)
        ChatReplyPolicy(mode=payload["chat_mode"])
        audio_mode = payload.get("audio_mode", "off")
        if (not isinstance(audio_mode, str) or audio_mode not in {"off", "transcribe", "dialog"}
                or audio_mode != "off" and not {"audio.receive", "chat.send"} <= set(payload["capabilities"])
                or audio_mode == "dialog" and payload["chat_mode"] == "off"):
            raise MeetError("meet_dialog_audio_policy_denied", 403)
        if payload["chat_mode"] != "off" and audio_mode != "dialog" and not {"chat.read", "chat.send"} <= set(payload["capabilities"]):
        ...
        context = {"lease_id": str(uuid.uuid4()), "runtime_id": str(uuid.uuid4()), "session_id": str(uuid.uuid4()),
            "room_id": self.authority.binding.profile.parse_invite(stored["invite_url"]), "owner_subject": principal.subject_id,
            "binding_task_id": parent, "deadline": int(self.clock()) + payload["duration_seconds"],
            "capabilities": sorted(payload["capabilities"]), "chat_mode": payload["chat_mode"],
            "audio_mode": audio_mode, "audio_job": None, "audio_count": 0}
        context["controls"] = initial_controls(context["capabilities"], context["chat_mode"], audio_mode, int(self.clock() * 1000))
        ...
        try:
            ...
            assignment = {"schema": "ananta.meet-dialog-assignment.v1", "task_id": task_id, "lease_id": scope.lease_id,
                "runtime_id": scope.runtime_id, "session_id": scope.session_id, "tenant_id": scope.tenant_id,
                "project_id": scope.project_id, "deadline": scope.deadline, "capabilities": list(scope.capabilities),
                "audio_mode": scope.audio_mode, "meeting": meeting}
            self.worker.start_dialog(assignment)
            self.authority.current(task_id, scope.lease_id, scope.runtime_id)
            return {"schema": "ananta.meet-dialog-start.v1", "task_id": task_id, "session_id": scope.session_id, "status": "connecting"}
        except Exception:
    ...
    def inspect(self, principal, project, task_id, *, stop=False):
        ...
        if task is None or task.task_kind != "meet_dialog_session" or task.tenant_id != principal.tenant_id or task.project_id != project:
        ...
        if stop and task.status == "in_progress":
            ...
            task = self.tasks.get_by_id(task_id)
        return {"schema": "ananta.meet-dialog-status.v1", "task_id": task_id,
                "status": task.status, "deadline": context.get("deadline", 0), "controls": context.get("controls"),
    ...
    def exchange(self, payload):
        ...
        if audio_job and (not scope.controls.audio.enabled or audio_job["control_revision"] != scope.controls.audio.revision
                          or not audio_job_current(audio_job, state, self.clock())):
            self.tasks.finish_audio(scope, audio_job, "failed"); audio_job = None
        renewal = None
        ...
        return {"schema": "ananta.meet-dialog-state.v1", "nonce": payload["nonce"], "authorization": state,
                "renewal": renewal, "audio_job": audio_job, "controls": asdict(scope.controls)}
    ...
    def chat(self, payload):
        ...
        if event.sent_at_ms < scope.controls.chat.since:
            return {"schema": "ananta.meet-dialog-answer.v1", "nonce": payload["nonce"], "code": "control_changed", "reply": None}
        current = CurrentDialogChatAuthority(self.authority, self.meet, ids, payload["meet_session_id"], event.sender_peer_id)
        admission = MeetChatAdmissionService(current, self.reservations, clock=self.clock).admit(raw)
        if admission.reservation is None:
            return {"schema": "ananta.meet-dialog-answer.v1", "nonce": payload["nonce"], "code": admission.code, "reply": None}
        principal = HubSourcePrincipal(scope.owner_subject, scope.tenant_id, scope.project_id, frozenset({"user"}))
        result = MeetChatReplyService(current, self.dispatches, self.authority.binding,
            self.media_worker, self.media_tasks, clock=self.clock).execute(principal, admission)
        return {"schema": "ananta.meet-dialog-answer.v1", "nonce": payload["nonce"], "code": "generated",
