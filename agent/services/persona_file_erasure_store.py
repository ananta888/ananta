"""Descriptor-confined removal under one fixed, operator-selected media profile."""
...
class PersonaFileErasureStore:
    def __init__(self, base_dir, *, profile):
        profiles = {
        ...
        self.kind, self.filename, self.maximum = profiles[profile]
    ...
    def erase(self, reference, expected_size, *, checkpoint):
        if (
            ...
            or not 0 < expected_size <= self.maximum
        ):
