...
def _bounded_payload(payload: Any, *, policy: EvolutionPolicy) -> Any:
    if payload is None:
        return None
    sanitized = _sanitize_persisted_payload(payload)
    try:
    ...
    if len(encoded.encode("utf-8")) <= policy.max_raw_payload_bytes:
        return sanitized
    semantic_preview = _semantic_payload_preview(sanitized)
    if semantic_preview:
        return {
            "_truncated": True,
...
def _semantic_payload_preview(payload: Any) -> dict[str, Any]:
...
def _sanitize_persisted_payload(payload: Any) -> Any:
    sanitized = _sanitize_details(payload) if isinstance(payload, dict) else payload
    return _redact_persisted_value(sanitized)
...
def _redact_persisted_value(value: Any) -> Any:
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            key_text = str(key)
            if _is_sensitive_persisted_key(key_text):
                redacted[key] = "***REDACTED***"
            else:
                redacted[key] = _redact_persisted_value(item)
        return redacted
    if isinstance(value, list):
