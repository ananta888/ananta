"""Bounded media discovery; scope and current preview authority stay separate."""
...
class PersonaAssetQuery:
    ...
    def query(self, principal, project, *, cursor, limit):
        if type(limit) is not int or not 1 <= limit <= 20:
            raise ValueError(f"persona_{self.kind}_query_limit_invalid")
        ...
        for artifact_id in candidates[:64]:
            ...
            try:
                reference = self.references.reference(principal, project, artifact_id)
                if (reference.kind, reference.tenant_id, reference.project_id, reference.artifact_id) != (
                    self.kind,
                    principal.tenant_id,
                    project,
                    artifact_id,
                ):
                    raise PermissionError("persona_query_reference_mismatch")
        ...
        if more and position == after:
            raise ValueError(f"persona_{self.kind}_query_deadline_exceeded")
