...
def recovery_dispatch_request_fingerprint(
    phase: str,
    payload: Any,
) -> str:
    ...
    proposal_context_bound = bool(
        normalized_phase == "execute"
        and data.get("recovery_proposal_context") is not None
    )
    run_evidence_context_bound = bool(
        data.get("recovery_run_evidence_context") is not None
    )
...
def recovery_accepted_result_digest(task: Any) -> str:
    ...
    payload = {
        ...
        "last_output_sha256": hashlib.sha256(
            output.encode("utf-8")
        ).hexdigest(),
...
def build_recovery_result_candidate(
    *,
    ...
    """Build the Hub-only handoff published atomically by ``result_guard``."""
    ...
    try:
        ...
        raise ValueError(
            "recovery_result_candidate_invalid"
        ) from exc
    if (
